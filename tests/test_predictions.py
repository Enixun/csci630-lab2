...
from lab2 import get_training_data, construct_hotter_daily_data, construct_precip_data
...
class DecisionTreeTests(TestCase):
  ...
  def test_hotter_today_random_forest(self):
    data, attributes = construct_hotter_daily_data(self.data, 'training')
    test_data, test_attrs, answers = construct_hotter_daily_data(self.data, 'testing')
    rf = RandomForest(10,len(test_attrs) - 2,4)
  ...
  def test_precipitation_today_decision_tree(self):
    data, attributes = construct_precip_data(self.data, 'training')
    dt = DecisionTree(data,list(attributes),10)
    self.assertIsNotNone(dt)
    test_data, test_attrs, answers = construct_precip_data(self.data, 'testing')
    number_correct = 0
    none_count = 0
    for i in range(len(test_data)):
      if dt.predict(test_data[i]) == answers[i]:
  ...
  def test_precipitation_today_random_forest(self):
    data, attributes = construct_precip_data(self.data, 'training')
    test_data, test_attrs, answers = construct_precip_data(self.data, 'testing')
    rf = RandomForest(10,len(test_attrs) - 2,4)
    rf.train(data,attributes)
